...
#		  particular directory and delete all the duplicate files present in the 
#		  particular folder, Ask user before deleting the files.
#
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
...
def FindDuplicate(duplicate):
	...
	if len(output) > 0:
		print("There are duplicate files present in specified directory")
	...
	if i == 0:
		...
		print("Total Number of duplicate files : ",i)
		return Arr
...
def DeleteDuplicate(Arr):
	...
	for i in range(len(Arr)):
		print("Do you Want to delete file ",Arr[i]," ?")
		prompt = input()
		if prompt.lower() == 'yes':
			iCnt = iCnt + 1
			os.remove(Arr[i])
	...
	print("Total Number of deleted  Files :",iCnt)

def main():
	Arr = {}

	print("-------------------------------  Automation Script for Directory Traversal and Duplicate Files Cleaner  ------------------------------")
